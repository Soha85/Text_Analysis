...
class KEM():
    ...
    def __lexical_diversity(self, sample_doc):
        ...
        return {"TTR": ld.ttr(flt), "RTTR": ld.root_ttr(flt), "LTTR": ld.log_ttr(flt), "massTTR": ld.maas_ttr(flt),
                "MSTTR": ld.msttr(flt),
                "MATTR": ld.mattr(flt), "HDD": ld.hdd(flt), "MTLD": ld.mtld(flt), "MTLD_MW": ld.mtld_ma_wrap(flt),
                "MTLD_MA": ld.mtld_ma_bid(flt)}
    ...
    def __lexical_richness(self, sample_doc):
        ...
        return {"TTR": lex.ttr, "RTTR": lex.rttr, "CTTR": lex.cttr, "MSTTR": lex.msttr(segment_window=5),
    ...
    def __std_coisne_similarity(self, sampledoc):
        ...
        for sent in sampledoc.sents:
            txt = ' '.join([re.sub(r'[' + string.punctuation + ']+', '', w.lower()) for w in sent.text.split(' ') if
                            (re.sub(r'[' + string.punctuation + ']+', '', w.lower()) not in stopwrds) and (
    ...
    def __std_GetLongSentInfo(self, sample_doc):
        ...
        for sent in sample_doc.sents:
            i = 0
            for w in nlp(sent.text,
                         disable=['textcat', 'ner', 'merge_entities', 'merge_subtokens', 'merge_noun_chunks']):
                if (not (w.is_punct)):
                    i = i + 1
            lst.append(i)
    ...
    def __std_GetwrdsSentInfo(self, sample_doc):
        res = []
        for sent in sample_doc.sents:
            res.append(sum(1 for w in nlp(sent.text, disable=['textcat', 'ner', 'merge_entities', 'merge_subtokens',
                                                              'merge_noun_chunks']) if not (w.is_punct)))
        return res
    ...
    def __matching_keywords(self, sample_doc, gldlst):
        ...
        for token in sample_doc:
            ...
            if (not (token.text.lower() in stopwrds) and not (token.is_punct)):
                if (token.is_alpha or token.ent_type_ in ('MONEY', 'TIME', 'DATE', 'CARDINAL') or token.pos_ in (
                'NUM')):
    ...
    def __Find_match_and_similarity(self, std_doc, gld_lst):
        ...
        for gld in gld_lst:
            ...
            res.append({'keywords': tmp, 'Count': len(tmp), 'Exact keywords': matched, 'Count-E': len(matched),
                        'Count-G': len(gld)})
        return res
